...
public enum ErrorCode {
    ...
    TOSS_PERSONAL_DATA_INVALID_BASE64("TOSS_PERSONAL_DATA_INVALID_BASE64", "Toss 개인정보 응답 형식이 올바르지 않습니다.", HttpStatus.BAD_GATEWAY),
    TOSS_PERSONAL_DATA_INVALID_PAYLOAD("TOSS_PERSONAL_DATA_INVALID_PAYLOAD", "Toss 개인정보 응답 구조가 올바르지 않습니다.", HttpStatus.BAD_GATEWAY),
    TOSS_PERSONAL_DATA_AUTHENTICATION_FAILED("TOSS_PERSONAL_DATA_AUTHENTICATION_FAILED", "Toss 개인정보 검증에 실패했습니다.", HttpStatus.BAD_GATEWAY),
    TOSS_PERSONAL_DATA_DECRYPTION_FAILED("TOSS_PERSONAL_DATA_DECRYPTION_FAILED", "Toss 개인정보 처리 중 오류가 발생했습니다.", HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    @Deprecated
    AD_OPEN_DAILY_LIMIT_EXCEEDED("AD_OPEN_DAILY_LIMIT_EXCEEDED", "오늘 광고 개봉 횟수를 모두 사용했습니다.", HttpStatus.TOO_MANY_REQUESTS),
    FREE_OPEN_LIMIT_EXCEEDED("FREE_OPEN_LIMIT_EXCEEDED", "이번 충전 주기의 무료 개봉 횟수를 모두 사용했습니다.", HttpStatus.TOO_MANY_REQUESTS),
    AD_OPEN_LIMIT_EXCEEDED("AD_OPEN_LIMIT_EXCEEDED", "이번 충전 주기의 광고 개봉 횟수를 모두 사용했습니다.", HttpStatus.TOO_MANY_REQUESTS),
    AD_REWARD_ID_REQUIRED("AD_REWARD_ID_REQUIRED", "광고 보상 식별자가 필요합니다.", HttpStatus.BAD_REQUEST),
    AD_REWARD_ALREADY_USED("AD_REWARD_ALREADY_USED", "이미 사용한 광고 보상입니다.", HttpStatus.CONFLICT),
    AD_REWARD_VERIFICATION_FAILED("AD_REWARD_VERIFICATION_FAILED", "광고 보상 검증에 실패했습니다.", HttpStatus.BAD_GATEWAY),
    KEYCAP_REWARD_NOT_AVAILABLE("KEYCAP_REWARD_NOT_AVAILABLE", "지급 가능한 키캡 보상이 없습니다.", HttpStatus.CONFLICT),

    ONBOARDING_TAP_INVALID("ONBOARDING_TAP_INVALID", "온보딩 탭 이벤트가 올바르지 않습니다.", HttpStatus.BAD_REQUEST),
    ONBOARDING_TAP_NOT_COMPLETED("ONBOARDING_TAP_NOT_COMPLETED", "온보딩 탭을 완료해야 합니다.", HttpStatus.BAD_REQUEST),
    ONBOARDING_TAP_SESSION_REUSED("ONBOARDING_TAP_SESSION_REUSED", "같은 온보딩 탭 세션으로 다른 요청을 처리할 수 없습니다.", HttpStatus.CONFLICT),
    ONBOARDING_REWARD_NOT_AVAILABLE("ONBOARDING_REWARD_NOT_AVAILABLE", "온보딩 보상을 사용할 수 없습니다.", HttpStatus.CONFLICT),
    ONBOARDING_ATTEMPT_EXPIRED("ONBOARDING_ATTEMPT_EXPIRED", "온보딩 보상 시도가 만료되었습니다.", HttpStatus.GONE),
    ONBOARDING_ATTEMPT_NOT_FOUND("ONBOARDING_ATTEMPT_NOT_FOUND", "온보딩 진행 기록을 찾을 수 없습니다.", HttpStatus.NOT_FOUND),
    ONBOARDING_ATTEMPT_NOT_OPENED("ONBOARDING_ATTEMPT_NOT_OPENED", "온보딩 보상 개봉 기록이 유효하지 않습니다.", HttpStatus.CONFLICT),

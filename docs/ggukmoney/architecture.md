...
- 상자 진행도는 `user_tap_session.session_valid_tap_count`와 `user_tap_session.next_box_target`을 원본으로 사용하고, 목표 도달 시 `keycap_box_account.box_balance`만 증가시킨다. 마지막 유효 탭 이후 `tap.box.session.idleTimeoutSeconds` 동안 탭이 없으면 누적 탭과 목표를 초기화하며, 유효 탭마다 만료 시각을 뒤로 민다.

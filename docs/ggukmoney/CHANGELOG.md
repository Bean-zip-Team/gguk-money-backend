...
- `submittedCount`에 `@Max(1000)`을 추가했다. 기존에는 일일 탭 상한이 배치 크기를 간접적으로 제한했으나, 상자 진행도를 상한에서 분리하면서 상자 지급 루프 반복 횟수와 `next_box_target`의 int 캐스팅이 클라이언트 입력에 그대로 노출되기 때문이다. 앱은 100탭 또는 30초마다 flush하므로 정상 배치는 상한에 닿지 않는다.

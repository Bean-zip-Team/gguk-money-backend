...
- 전체 기기 로그아웃·정지·탈퇴는 `auth:revoke:user:{userId}`의 millisecond revoke 시각으로 기존 Access Token을 즉시 차단한다.
...
- Access Token 없이 호출하면 Toss identity 기준 일반 로그인 또는 신규 회원 생성을 수행한다.
...
오류: `TOSS_AUTH_FAILED`, `USER_MERGE_CONFLICT`, `AUTH_SESSION_EXPIRED`.
...
- `auth:revoke:user:{userId}`에 현재 epoch millis인 `revokedAtMillis`와 사유 `LOGOUT_ALL`을 저장한다.

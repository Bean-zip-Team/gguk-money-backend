...
- `FREE`는 성공 시 `box_balance`를 1 차감하고 공통 1시간 주기의 `free_open_used_count`를 1 증가시킨다.
- `ADVERTISEMENT`는 성공 시 `box_balance`를 1 차감하고 공통 1시간 주기의 `ad_open_used_count`를 1 증가시키며 검증된 `ad_reward_id`를 소비한다. 광고 Provider 신규 검증 구현은 이번 범위에 포함하지 않는다.
...
- 후보가 없으면 `409 KEYCAP_REWARD_NOT_AVAILABLE`을 반환하고 `box_balance`, `free_open_used_count`, `ad_open_used_count`, `ad_reward_id`를 소비하지 않으며 `keycap_box_open` 이력도 생성하지 않는다.
...
2. `keycap_box_account`를 잠그고 공통 1시간 주기 만료를 `open_cycle_started_at` 기준으로 반영한다.
3. 상자 보유 여부와 개봉 방식별 공통 주기 사용 한도를 검증한다.
4. 미완성 활성 키캡 후보를 조회한다.
5. 후보가 없으면 `KEYCAP_REWARD_NOT_AVAILABLE`을 반환한다.
6. 후보 중 하나를 균등 랜덤으로 선택한다.
7. 상자, 개봉 방식별 사용 횟수, 광고 보상 등 필요한 자원을 차감한다.
...
- `GET /api/keycap-boxes/history`는 별도 `Idempotency-Key` Header를 사용하지 않는다.

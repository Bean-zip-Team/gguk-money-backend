...
| `A` | `CONFIRMED` | 회원/인증 | `GET` | `/api/v1/members/me/merge-status` | 병합 상태 조회 |
| `A` | `CONFIRMED` | 회원/인증 | `POST` | `/api/v1/members/me/merge-retry` | 병합 재시도 |
...
### GET /api/v1/members/me/merge-status
...
### POST /api/v1/members/me/merge-retry
...
- `GET /api/v1/home.onboarding.milestonesGranted`는 현재까지 완료된 milestone 목록이다.
...
- `POST /api/v1/taps/batches.onboarding.milestonesGranted`는 해당 요청에서 새로 지급된 milestone 목록이다. `GET /api/v1/home`의 누적 목록과 의미가 다르다.

# 꾹머니 14개 테이블 MVP API 계약

이 문서는 14개 테이블 Persistence MVP가 지원할 HTTP API 범위를 정의한다. 인증 세부 흐름은 [auth-lifecycle.md](auth-lifecycle.md)를 따르고, 프론트 연동용 상세 Request/Response와 JSON 예시는 [frontend-api-guide.md](frontend-api-guide.md)를 따른다.
...
| 상태 | API |
|---|---|
| 구현 확인 | `POST /api/v1/auth/toss/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`, `POST /api/v1/auth/logout-all`, `POST /api/v1/auth/toss/unlink-webhook`, `GET /api/v1/members/me`, `PATCH /api/v1/members/me`, `POST /api/v1/members/me/withdrawal`, `GET /api/v1/app-config`, `GET /api/v1/keycaps`, `GET /api/v1/keycaps/me`, `PUT /api/v1/keycaps/{keycapId}/equip`, `GET /api/v1/keycap-boxes/status`, `POST /api/v1/keycap-boxes/open`, `GET /api/v1/keycap-boxes/history`, `POST /api/v1/onboarding/keycap-boxes/open`, `POST /api/v1/tap/batches`, `POST /api/v1/boosters/activate`, `GET /api/v1/boosters/current` |
...
- 아직 구현되지 않은 목록 API의 `page/size` 방식은 프론트 Mock과 타입 설계를 위한 초안이며 cursor 방식과 최종 선택이 필요하다.
...
> 구현 확인: 신규 사용자 온보딩 정산은 로그인 요청의 선택 필드 `onboardingAttemptId`로 처리한다. 프론트가 `keycapId`, `shardCount`, `completed`, `tapCount`, `rewardPoint`, 상자 개봉 결과 전체를 보상 원본으로 전달하는 구조는 사용하지 않는다.
...
- 온보딩 보상 키캡 코드, 포인트 수량, attempt TTL은 `app_config`에서 조회한다. 누락, 비활성 키캡, 음수 포인트, 0 이하 TTL은 `ONBOARDING_REWARD_NOT_AVAILABLE`로 처리한다.
...
- Redis Session 저장 실패 후 재로그인에는 Toss 정책상 새로운 `authorizationCode`가 필요할 수 있다. 보상 멱등성의 원본은 Redis가 아니라 `onboarding_reward_attempt`, `point_ledger.idempotency_key`, `user_keycap` Unique 제약이다.
...
- 현재 구현은 `ADVERTISEMENT`를 `ADVERTISEMENT_OPEN_NOT_SUPPORTED`로 차단하며 광고 검증 Provider 연동과 성공 처리는 포함하지 않는다.
...
- `GET /api/v1/keycap-boxes/history`는 별도 `Idempotency-Key` Header를 사용하지 않는다.
- `cursor`는 서버가 발급한 opaque 문자열이며 내부적으로 `opened_at DESC, id DESC` 정렬 위치를 표현한다.
- `size` 기본값은 20이고 허용 범위는 1~100이다. 범위를 벗어나거나 cursor가 올바르지 않으면 `COMMON_VALIDATION_ERROR`를 반환한다.
- 응답은 `content`, `nextCursor`, `hasNext`를 반환하며 전체 개수는 계산하지 않는다.
- 각 항목은 `boxOpenId`, `openMethod`, `keycapId`, `shardCount`, `completed`, `openedAt`만 노출하고 내부 BIGINT ID, 사용자 ID, 멱등키, 요청 해시, 광고 보상 ID는 노출하지 않는다.

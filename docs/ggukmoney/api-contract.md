...
- 모든 개봉은 `box_balance`를 1 차감한다.
- `FREE`는 `box_balance`와 함께 `free_open_ticket_count`를 1 차감한다.
- `ADVERTISEMENT`는 `box_balance`를 1 차감하고 검증된 `ad_reward_id`를 소비한다. 광고 검증 Service가 구현되기 전에는 미지원 오류로 처리하며 자원을 차감하지 않는다.
- 광고 검증이 구현된 뒤에는 `ad_reward_id`가 필수이며, `ad_reward_id` 중복 사용을 금지한다.
- 키캡 보상은 활성 키캡에서 선택하고 기본 지급 조각 수는 1개다. 완성 전환 시 `completed_at`을 기록한다.

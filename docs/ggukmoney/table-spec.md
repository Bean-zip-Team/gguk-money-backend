...
| `device_public_id` | UUID | Y | | | | optional metadata. 값이 없으면 null |
...
- 보관/익명화/삭제: 보안 감사 목적 기간 동안 보관하고 탈퇴 시 사용자 식별자는 법무 보관 정책에 따라 익명화한다.
...
- 상태 의미:
  ...
  - `PROCESSING`: 정산 작업 진행 중 표시이며 동시 실행 방지용. 낙관적 `version` 또는 조건부 update로 전환한다.
  - `COMPLETED`: accepted tap 반영, 신규 사용자 포인트 지급, 신규 사용자 키캡 지급, 최종 응답 데이터 확정.
  - `FAILED`: 재시도 가능한 로컬 정산 실패가 별도 recovery transaction으로 기록된 상태. `failure_code`로 재시도 가능 코드와 수동 확인 필요 코드를 구분한다.
